...
def writeToReports(input_filename, image):
    ...
    table_time = []
    times, mean_time = runTimes(python_grayscale, image)
    a, b, c = times
    column = 'python_color2gray', a, b, c, mean_time
    table_time.append(column)

    e = datetime.datetime.now() # Finds the time

    file_name = 'python_report_color2gray.txt'
    ...
    file.write(f"Average runtime running python_color2gray after 3 runs: {mean_time} s \n")
    file.write(f"Timing performed using: time module")

    file.close()


def python_grayscale(image):
...
def main(input_filename, output_filename):
    ...
    gray_image = python_grayscale(image) # Convert it to gray image
...
if __name__ == '__main__':
    ...
    main('../rain.jpg', 'python_rain_grayscale.jpg')
